...
class Miyagi(plugins.Plugin):
                    ...
                    for secname, sec in [["AI", self.agent._config['ai']],
                        ...
                        for p in sec:
                            if type(sec[p]) in [int, str, float]:
                                ...
                                ret += '<td><input type=text id="newval_%s_%s" name="newval_%s_%s" size="5"></td>' % (
                                sec, p, sec, p)
    ...
    def on_ui_setup(self, ui):
        ...
        orientation = self.options["orientation"] if "orientation" in self.options else "vertical"
        miyagi_pos = None
        epoch_pos = None
        try:
            pos = [int(x.strip()) for x in self.options["position"].split(",")] if "position" in self.options else None
        except Exception:
            logging.error("[miyagi] Could not process set position, using default.")
            logging.debug("[miyagi] Traceback:\n" + traceback.format_exc())
            pos = None
        if pos and len(pos) == 2:
            if orientation == "vertical":
                miyagi_pos = (pos[0], pos[1] + line_spacing)
                epoch_pos = (pos[0], pos[1])
            elif orientation == "horizontal":
                miyagi_pos = (pos[0] + 60, pos[1])
                epoch_pos = (pos[0], pos[1])
        else:
            logging.info("[better_miyagi] Position is not set or is incorrectly set, using default.")
            if orientation == "vertical":
                miyagi_pos = (69, 79)
                epoch_pos = (69, 71)
            elif orientation == "horizontal":
        ...
        try:
            ...
            logging.error("[miyagi] Could not load in UI, traceback:\n" + traceback.format_exc())
    ...
    def on_ready(self, agent):
        ...
        self._laziness = self.agent._config['ai']['laziness'] if 'laziness' not in self._mconfig else self._mconfig[
            'laziness']
